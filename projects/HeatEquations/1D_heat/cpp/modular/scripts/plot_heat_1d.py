...
def plot_scheme_set(dt):
    tag = make_dt_tag(dt)
    schemes = ["ftcs_explicit", "dufort", "ftcs_implicit", "cn"]
    markers = ["o", "s", "^", "d"]
...
def plot_error_compare(dt, t_target):
    tag = make_dt_tag(dt)
...
def plot_convergence_only(t_target):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python3 scripts/plot_heat_1d.py scheme <dt>")
        print("  python3 scripts/plot_heat_1d.py error <dt> <t_target>")
        print("  python3 scripts/plot_heat_1d.py convergence <t_target>")
        sys.exit(1)
    ...
    if mode == "scheme":
        if len(sys.argv) != 3:
            sys.exit("scheme mode requires: <dt>")
        dt = float(sys.argv[2])
        plot_scheme_set(dt)

    elif mode == "error":
        if len(sys.argv) != 4:
            sys.exit("error mode requires: <dt> <t_target>")
        dt = float(sys.argv[2])
        t_target = float(sys.argv[3])
        plot_error_compare(dt, t_target)

    elif mode == "convergence":
        if len(sys.argv) != 3:
            sys.exit("convergence mode requires: <t_target>")
        t_target = float(sys.argv[2])
        plot_convergence_only(t_target)
