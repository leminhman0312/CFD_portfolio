...
void sim(double dx, double dt, int imax, double t0, double tb, double F) {
  ...
  std::snprintf(ferr_exp, sizeof(ferr_exp), "data/error_ftcs_explicit_%s.txt",
                tag);
  std::snprintf(ferr_imp, sizeof(ferr_imp), "data/error_ftcs_implicit_%s.txt",
                tag);
  ...
  for (int k = 0; k <= 4; k++) {
    ...
    exact_solution_profile(imax, x.data(), t, alpha, L, tb, t0, 200,
                           Tex.data());
...
void plot(double dt) {
  ...
  std::snprintf(cmd, sizeof(cmd), "python3 scripts/plot_heat_1d.py scheme %.6f",
                dt);

  int status = std::system(cmd);
  if (status != 0) {
    std::fprintf(stderr, "Python scheme plotting failed.\n");
    std::exit(1);
  }
}
// -----------------------------------------------------------------------------
...
void plot_error_schemes(double dt, double t_target) {
  ...
  std::snprintf(cmd, sizeof(cmd),
                "python3 scripts/plot_heat_1d.py error %.6f %.6f", dt,
                t_target);

  int status = std::system(cmd);
  if (status != 0) {
    std::fprintf(stderr, "Python error plotting failed.\n");
    std::exit(1);
  }
}
...
// -----------------------------------------------------------------------------
void convergence_study(double dx, int imax, double alpha, double t0, double tb,
                       double t_target) {
  const int ndt = 5;
...
void plot_convergence(double t_target) {
  ...
  std::snprintf(cmd, sizeof(cmd),
                "python3 scripts/plot_heat_1d.py convergence %.6f", t_target);
