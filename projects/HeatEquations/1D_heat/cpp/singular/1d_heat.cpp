...
static void write_block_T(FILE* f, double t, int imax, const double x[],
                          const double T[]) {
...
static void write_block_error(FILE* f, double t, int imax, const double x[],
                              const double T[], const double Texact[]) {
  std::fprintf(f, "# t = %.2f hr\n# x\terr\n", t);
  for (int i = 1; i <= imax; i++) {
    std::fprintf(f, "%.6f\t%.6f\n", x[i], (T[i] - Texact[i]));
  }
...
static void exact_solution_profile(int imax, const double x[], double t,
                                   double alpha, double L, double tb, double t0,
...
static void FTCS_explicit(int nmax, double F, double tb, double t0, int imax,
                          double Tout[]) {
...
static void implicit_interior_one_step(double F, double tb, double t0, int imax,
                                       double u_full[]) {
  int N = imax - 2;
...
static void DufortFrankel(int nmax, double F, double tb, double t0, int imax,
                          double Tout[]) {
  const double dval = 2.0 * F;

  std::vector<double> un_m1(imax + 1), un(imax + 1), un_p1(imax + 1),
      u1(imax + 1);
  ...
  for (int step = 1; step <= nmax - 1; step++) {
    for (int i = 2; i <= imax - 1; i++) {
      un_p1[i] = ((1.0 - dval) * un_m1[i] + dval * (un[i + 1] + un[i - 1])) /
                 (1.0 + dval);
    }
    un_p1[1] = tb;
    un_p1[imax] = tb;
...
static void FTCS_implicit(int nmax, double F, double tb, double t0, int imax,
                          double Tout[]) {
  std::vector<double> a(imax + 1, 0.0), b(imax + 1, 0.0), c(imax + 1, 0.0),
      d(imax + 1, 0.0), un(imax + 1, 0.0);
...
static void CrankNicolson(int nmax, double F, double tb, double t0, int imax,
                          double Tout[]) {
  const double dnc = F / 2.0;

  std::vector<double> a(imax + 1, 0.0), b(imax + 1, 0.0), c(imax + 1, 0.0),
      d(imax + 1, 0.0), u0(imax + 1, 0.0), uhalf(imax + 1, 0.0);
...
static void sim(double dx, double dt, int imax, double t0, double tb,
                double F) {
  char tag[8];
  make_dt_tag(dt, tag);
  ...
  if (!uexp || !udf || !uimp || !ucn || !uex || !ueexp || !uedf || !ueimp ||
      !uecn) {
    std::perror("fopen");
    std::exit(1);
  }
  ...
  std::vector<double> Tex(imax + 1), Texp(imax + 1), Tdf(imax + 1),
  ...
  for (int k = 0; k <= 4; k++) {
    ...
    exact_solution_profile(imax, x.data(), t, alpha_loc, L, tb, t0, 200,
                           Tex.data());
...
static void convergence_study(double dx, int imax, double alpha, double t0,
                              double tb, double t_target) {
  const int ndt = 5;
  ...
  for (int k = 0; k < ndt; k++) {
    ...
    exact_solution_profile(imax, x.data(), t_target, alpha, L, tb, t0, 200,
                           Tex.data());
    ...
    std::fprintf(
        ...
        L2e, L2i, L2d, L2c, Lie, Lii, Lid, Lic);
  }
...
// ============================================================================
//  plot
//  Calls python scripts to generate png files into plot/
// ============================================================================

static void plot(double dt, double t_target) {
  char cmd[512];

  std::snprintf(cmd, sizeof(cmd), "python3 plot_heat_1d.py %.6f %.6f", dt,
                t_target);

  int status = std::system(cmd);

  if (status != 0) {
    std::fprintf(stderr, "Python plotting script failed.\n");
    std::exit(1);
  }
}
