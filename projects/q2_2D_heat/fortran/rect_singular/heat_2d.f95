...
program heat2d
  ...
  dt_explicit_given = 0.000001_real64
  ...
  if (do_sim .or. do_all .or. do_anim) then
    call sim(u0, t_end, deltax, deltay, alpha, dt_implicit, dt_explicit_given, t1, t2, t3, t4, anim_on=do_anim)
  end if
...
contains
  ...
  subroutine sim(u0, t_end, deltax, deltay, alpha, dt_implicit, dt_explicit_given, t1, t2, t3, t4, anim_on)
    ...
    if (sum <= 0.5_real64) then
       ...
       ! ----------------------------------------------------
       ! Unstable dt case: animate this too (blow up movie)
       ! ----------------------------------------------------
       ...
       ! ----------------------------------------------------
       ! Safe dt case: animate as before
       ! ----------------------------------------------------
  ...
  subroutine FTCS_Explicit(u0, t_end, deltax, deltay, dt, alpha, t1, t2, t3, t4, u, anim_on, frame_every)
    ...
    ! exact number of steps that will be executed
    ...
    do n = 1, nsteps
       ...
       call animation_step(n, nsteps, 'Explicit FTCS', do_anim, every)
       if (do_anim) then
