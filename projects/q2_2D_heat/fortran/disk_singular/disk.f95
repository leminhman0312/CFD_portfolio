...
!
! Central hot disk source is clamped every time step.
!
! Boundary condition modes
!   Mode 1 all Neumann zero gradient on all sides
!   Mode 2 mixed BC left wall Dirichlet fixed cooling, other 3 sides Neumann
!   Mode 3 full Dirichlet using t1 t2 t3 t4
!
! Implicit ADI tridiagonal end equations are consistent with the chosen BC.
! ============================================================================
...
program disk2d
   ...
   ! Boundary condition mode flags
   logical :: use_neumann_bc
   logical :: use_mixed_bc
   real(real64) :: t_left_fixed
   ...
   ! Mode 2 mixed BC left fixed cooling, other 3 sides Neumann
   ...
   ! If you use Mode 2, this is the fixed cooling temperature on the left wall
   t_left_fixed = 0.0_real64

   allocate(u0(imax, jmax))
   call initializeField(imax, jmax, t0, t1, t2, t3, t4, u0)

   if (use_source) call clamp_circle(u0, deltax, deltay, r_source, t_source)

   if (do_sim .or. do_all .or. do_anim) then
      call sim(u0, t_end, deltax, deltay, alpha, dt_implicit, dt_explicit_given, t1, t2, t3, t4, &
         use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, anim_on=do_anim)
   end if

   if (do_conv .or. do_all) then
      call convergence(u0, t_end, deltax, deltay, alpha, t1, t2, t3, t4, &
         use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed)
   end if
...
contains
   ...
   subroutine apply_bc_mixed(u, left_fixed, t_left, right_fixed, t_right, bottom_fixed, t_bottom, top_fixed, t_top)
      real(real64), intent(inout) :: u(:,:)
      logical, intent(in) :: left_fixed, right_fixed, bottom_fixed, top_fixed
      real(real64), intent(in) :: t_left, t_right, t_bottom, t_top
      integer :: im, jm
   ...
   end subroutine apply_bc_mixed

   subroutine apply_bc(u, use_neumann, use_mixed, t1, t2, t3, t4, tleft)
      real(real64), intent(inout) :: u(:,:)
      logical, intent(in) :: use_neumann, use_mixed
      real(real64), intent(in) :: t1, t2, t3, t4
      real(real64), intent(in) :: tleft

      if (use_neumann) then
         call apply_bc_neumann_zero(u)
      else if (use_mixed) then
         call apply_bc_mixed(u, &
            left_fixed=.true.,  t_left=tleft, &
            right_fixed=.false., t_right=0.0_real64, &
            bottom_fixed=.false., t_bottom=0.0_real64, &
            top_fixed=.false.,   t_top=0.0_real64)
      else
         u(1,:) = t2
         u(size(u,1),:) = t4
         u(:,1) = t1
   ...
   subroutine sim(u0, t_end, deltax, deltay, alpha, dt_implicit, dt_explicit_given, t1, t2, t3, t4, &
      use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, anim_on)
      ...
      real(real64), intent(in) :: t1, t2, t3, t4
      logical, intent(in) :: use_source
      real(real64), intent(in) :: r_source, t_source
      logical, intent(in) :: use_neumann_bc, use_mixed_bc
      real(real64), intent(in) :: t_left_fixed
      ...
      if (do_anim) call clear_animation_frames()

      call FTCS_implicit_ADI(u0, nmax_implicit, deltax, deltay, dt_implicit, alpha, t1, t2, t3, t4, &
         use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u_implicit, do_frames=do_anim, frame_every=1)
      ...
      if (sum <= 0.5_real64) then
         ...
         call FTCS_Explicit(u0, t_end, deltax, deltay, dt_explicit_given, alpha, t1, t2, t3, t4, &
            use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u_explicit, anim_on=do_anim, frame_every=1)
         ...
         call FTCS_Explicit(u0, t_end, deltax, deltay, dt_explicit_given, alpha, t1, t2, t3, t4, &
            use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u_fail, anim_on=do_anim, frame_every=1)
         ...
         call FTCS_Explicit(u0, t_end, deltax, deltay, dt_safe, alpha, t1, t2, t3, t4, &
            use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u_safe, anim_on=do_anim, frame_every=1)
   ...
   subroutine convergence(u0, t_end, deltax, deltay, alpha, t1, t2, t3, t4, &
      ...
      call FTCS_implicit_ADI(u0, nmax_ref, deltax, deltay, dt_ref, alpha, t1, t2, t3, t4, &
         use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u_ref)
      ...
      do k = 1, n_dt
         ...
         call FTCS_implicit_ADI(u0, nmax, deltax, deltay, dt, alpha, t1, t2, t3, t4, &
            use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u)
   ...
   subroutine FTCS_Explicit(u0, t_end, deltax, deltay, dt, alpha, t1, t2, t3, t4, &
      use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, u, anim_on, frame_every)
      ...
      if (do_anim) then
         ...
         call ensure_dir('animation/dat')
         call ensure_dir('animation/png')
         write(output_unit,'(/,A,I0)') 'Explicit FTCS animation: total steps = ', nsteps
      ...
      do n = 1, nsteps
         ...
         call apply_bc(u_new, use_neumann_bc, use_mixed_bc, t1, t2, t3, t4, t_left_fixed)
         if (use_source) call clamp_circle(u_new, deltax, deltay, r_source, t_source)
      ...
      if (do_anim) write(output_unit,'(A)') ''
   ...
   subroutine animation_step(step, nmax, scheme, anim_on, every)
      ...
      write(output_unit,'(A,A,I0,A,I0,A)', advance='no') &
         char(13), trim(scheme)//': Frame ', step, ' / ', nmax, repeat(' ', 30)
      flush(output_unit)
   end subroutine animation_step

   subroutine FTCS_implicit_ADI(u0, nmax, deltax, deltay, dt, alpha, t1, t2, t3, t4, &
      use_source, r_source, t_source, use_neumann_bc, use_mixed_bc, t_left_fixed, &
      u, do_frames, frame_every)
      use, intrinsic :: iso_fortran_env, only: output_unit
      real(real64), intent(in) :: u0(:,:)
      ...
      ax = 0.0_real64; bx = 0.0_real64; cx = 0.0_real64; dx = 0.0_real64; solx = 0.0_real64
      ay = 0.0_real64; by = 0.0_real64; cy = 0.0_real64; dy = 0.0_real64; soly = 0.0_real64
      ...
      if (frames_on) then
         ...
         call ensure_dir('animation/dat')
         call ensure_dir('animation/png')
         write(output_unit,'(/,A,I0)') 'Implicit ADI animation: total steps = ', nmax
         call write_frame(0, u, deltax, deltay, 0.0_real64, scheme)
      ...
      do n = 1, nmax
         ...
         call apply_bc(u, use_neumann_bc, use_mixed_bc, t1, t2, t3, t4, t_left_fixed)
         if (use_source) call clamp_circle(u, deltax, deltay, r_source, t_source)
         ...
         do j = 2, jmax2-1
            ...
            if (.not. use_neumann_bc) then
               if (use_mixed_bc) then
                  cx(2) = cx(2) + (fx/2.0_real64) * t_left_fixed
               else
         ...
         call apply_bc(u_dummy, use_neumann_bc, use_mixed_bc, t1, t2, t3, t4, t_left_fixed)
         if (use_source) call clamp_circle(u_dummy, deltax, deltay, r_source, t_source)
      ...
      if (frames_on) write(output_unit,'(A)') ''
